...
The query param defaults to `editing` and is set with
`data-edit-mode-param-value`. Give each region its own name when a page has more
than one — under a shared param they read the same flag and enter together:

```erb
<section data-controller="edit-mode" data-edit-mode-param-value="editing_widgets">…</section>
<section data-controller="edit-mode" data-edit-mode-param-value="editing_layout">…</section>
```
