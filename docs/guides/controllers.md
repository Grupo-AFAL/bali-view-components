...
| [`number-format`](#number-format) | Groups the thousands of an amount as it is typed | any numeric FormBuilder family with `delimited: true` |
...
That is also why `delimited: true` is opt-in on every family: turning the
delimiter on costs a `number_group` its native type, along with the `min`, `max`
and `step` the browser only enforces there — and, on every family, it changes
...
- **Deleting a delimiter deletes the digit behind it.** Backspace over a
  ...
  nothing forever. Deletions carrying a modifier (`Option`, `Ctrl`, `Cmd`) are
  left to the browser, which deletes by word and by line.
- **A half-typed decimal is completed on blur.** `1500.` becomes `1,500` and
  ...
  way out.
- **The value found on connect is never touched.** Whether `1.500` is a machine
  number or something the typist wrote is not decidable here: in Spanish the dot
  is the *delimiter*, so one reading submits 1.5 and the other 1500, and each
  corrupts the case it guessed wrong. Only the server knows, so the FormBuilder
  groups the initial value itself — in the locale of the request, and only when
  the model handed over a number rather than a string the typist submitted.
...
to 1 — which is why the delimiter is never on by default.
