...
| `WorkflowSteps` | **own classes**, not `steps step-*` | Axis is `orientation:` too, with a third value `:rail`. daisyUI's `.step` takes its status from position and cannot say "step 2 was rejected" |
