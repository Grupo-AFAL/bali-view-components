...
// Puts the grid into an edit mode and remembers it in the URL: toggles a class,
// swaps the control that enters for the one that leaves, marks the cards `inert`,
...
export class WidgetGridEditModeController extends Controller {
  ...
  // A turbo-stream can replace a card WHILE the mode is on, and
  // `editingValueChanged` does not fire again for the new element — so without
  // this the replacement arrives live inside a dimmed, inert grid: tabbable, and
  // one Enter away from navigating out of an arrangement in progress. The grid
  // does exactly that when a resize changes a card's shape.
