  ...
  module Testing
    ...
    module StoreContract
      ...
      def assert_bali_store_contract(store)
        ...
        # `arrange([])` clears at least the rows THIS STORE'S OFFERING COVERS —
        # hand this a SCRATCH store scoped to a throwaway owner or dashboard_key,
        # never one holding rows you want to keep.
        store.arrange([])
        store.choose([])
      end
    end
