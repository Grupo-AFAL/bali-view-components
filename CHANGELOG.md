...
- **DataTable SavedViews** - the active view is visibly highlighted again. daisyUI 5 renamed the menu item marker from `active` to `menu-active` and dropped the old `.menu li > .active` rule, so the component kept applying a class that styled nothing — and the tests kept passing because they asserted the class rather than the one that renders. (`Bali::SideMenu` is unaffected: it ships its own `.active` CSS.)
