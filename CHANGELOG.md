...
- **El FormBuilder deja de filtrar sus propias opciones al DOM, y deja de mutar el hash del que llama.** Los helpers leían `label:`, `help:`, `mode:`, `control_class:` y otras 30 claves con un `delete` disperso por módulo, y lo que sobrevivía a ese `delete` se lo pasaban tal cual a Rails — que reenvía al elemento cualquier clave que no reconoce. El resultado, medido sobre render real, era `<input label="Nombre" help="Máx 80" control_class="w-full" mode="range" pattern_type="number_with_commas">` en **20 de 20** helpers de entrada, incluidos `select_field`, `slim_select_field` y `time_zone_select`. Ahora existe una sola lista, `HtmlUtils::RESERVED_OPTIONS`, y un solo punto de extracción, `html_attributes`, por el que pasa todo lo que delega en Rails; ningún módulo vuelve a hacer su propio `delete`. La lista distingue a propósito entre lo que Bali consume y lo que es atributo HTML de verdad: `class`, `type`, `value`, `required`, `placeholder`, `min`, `max`, `step`, `multiple`, `disabled` y `data` siguen llegando al elemento, y `size` y `color` **no** están en la lista global porque en un `text_field` son atributos legítimos aunque en un checkbox nombren variantes de daisyUI — esos se extraen junto al helper que les da ese significado.

  **La mutación era el defecto más caro de los dos.** `field_options` escribía `options[:class] = "input input-bordered w-full #{options[:class]}"` sobre el hash del llamador, así que reutilizar un mismo hash entre dos campos acumulaba las clases del primero en el segundo, y un hash congelado reventaba con `FrozenError` en 12 de 20 helpers. Desaparecen todos los `with_defaults!` sobre hashes del usuario y todos los `delete` sobre el hash recibido. Ojo con el caso que un `dup` no arregla: `prepend_action` y compañía mutan **el hash `:data` anidado**, que `dup`, `except` y `merge` siguen compartiendo con el llamador — de ahí `dup_options`, que copia esa clave, en las siete rutas que llaman a `prepend_*`.

  **Qué se rompe para un host que actualiza.** Si algún CSS o algún test de integración selecciona por esos atributos inválidos (`input[mode="range"]`, `[control_class]`, `[help]`), deja de encontrarlos: ya no se emiten. Es improbable —son atributos que ningún validador acepta y que nadie escribiría a mano— pero es el único cambio observable en el HTML de un formulario que ya funcionaba; el resto del markup es idéntico, verificado comparando los 464 previews de Lookbook antes y después. Dos efectos secundarios que sí cambian comportamiento: `submit_actions` vuelve a respetar `show_cancel_button?` cuando `Bali.native_app` está activo y se pasa `modal:` (antes `submit` borraba `:modal` del hash antes de que la comprobación lo leyera, así que el botón de cancelar se mostraba siempre), y `submit` con `Bali.native_app` deja de emitir `modal="true"` y `drawer="true"` en el `<button>`.
