...
- **DataTable listing identity** - one listing now has ONE name. `table_id:` is removed from `with_column_selector` and `with_saved_views` (passing it raises `ArgumentError: unknown keyword: :table_id`); DataTable resolves the identity itself — explicit `id:`, else `filter_form.storage_id`, else a random hex — sanitizes it into a valid CSS identifier (case preserved; a leading digit gets a `listing-` prefix, because `#123 table` makes `querySelector` throw), and renders its container with it. The column selector now targets `#<id> table` instead of the table element, and both controls derive their strings from the same place (`Bali::DataTable::ListingIdentity`): the saved-views controller locates the selector by comparing that exact attribute and reads its columns from the matching key, so two separate derivations lost the columns on save without failing anywhere. With no stable identity (the random hex) column persistence turns itself OFF instead of writing a key that can never be read back. Two consequences to migrate: stored column preferences reset once (the key goes from `bali:columns:<table_id>` to `bali:columns:<id>` — which is also the fix for `/movies` and `/admin/movies` sharing one memory through `#movies-table`), and the container id changes from `data-table-<scope cache_key>` to the resolved id, so a host doing `turbo_stream.replace "data-table-#{@filter_form.id}"` must switch to `turbo_stream.replace @filter_form.storage_id` — Turbo finds no target otherwise and applies the stream to nothing, silently.

- **DataTable view switch** - `with_view_switch` puts a `Bali::ViewSwitch` in the toolbar and builds the hrefs itself: each view declares `value:` (what travels in `?view=`, renamed from the internal `data_display_mode`) and the DataTable merges the current query string, dropping `page` and KEEPING `saved_view` — these links are navigation, not a filter submit, so filters, sorting, grouping and the applied view survive a mode change. `dt.display_mode` returns the value validated against the declared views: an unknown `?view=` falls back to the first declared one instead of leaving the listing empty. This retires the legacy `ActionsPanel` toggle and its export dropdown — `with_actions_panel(grid_display_mode_enabled:)` and `(export_formats:)` now raise `ArgumentError: unknown keyword`, replaced by `with_view_switch` and `with_export` — which also closes #653: the toggle built its links with `Utils::Url#add_query_params`, whose Symbol-over-String merge duplicated a param already present in the URL, and that code is no longer on the path.

- **DataTable selection and bulk actions** - `with_actions_panel` is removed, and with it `Bali::DataTable::ActionsPanel` and `Bali::DataTable::Action` (its only caller): calling it now raises `NoMethodError`. Its replacement is `with_bulk_actions`, a `Bali::BulkActions(variant: :toolbar)` contextual row — counter, actions, clear button — that REPLACES the toolbar row while a selection exists and restores it when it is cleared, so the two never compete for the same space; this also retires the `hidden md:block` desktop panel plus its `md:hidden` mobile COPY, the duplication pattern that made two Stimulus controllers drive the same listing. `Bali::BulkActions` gains `variant: :floating | :toolbar` and `standalone:`; the floating bar is unchanged and stays available outside a DataTable. `Bali::Table` gains `selectable: true`, which renders the checkbox column and the select-all header that every app hand-wrote until now — the `<tr>` is the selectable item (it carries `record_id:`, required, and the `selected` class), and it is mutually exclusive with the legacy `bulk_actions:` array (`IncompatibleOptions`). The `bulk-actions` controller goes on the DataTable CONTAINER, never on the bar: two nested controllers of the same identity split the targets between them and the bar would never see the rows, silently. The controller itself now derives `selectedIds` from the DOM instead of counting incrementally — that is what lets select-all, clear and a Turbo cache restore stay in sync with what is on screen — and gains `toggleItem`, `toggleAll`, `clear`, an indeterminate select-all and server-rendered plural labels (no i18n interpolation in JS). Selection is per page and resets on pagination, filtering or a mode change, all of which re-render the node that carries the controller.

- **DataTable toolbar overflow** - below `sm` (640px) the toolbar folds its secondary controls into a `⋯` menu and unfolds them on the way back, MOVING the nodes instead of duplicating them: two copies of the column selector are two Stimulus controllers driving one table, and two copies of saved views duplicate the ids in its rename forms — the bug fixed in #669. The new `toolbar-overflow` controller is registered by `registerAll` and exported from the package root. Survival order (`OVERFLOW_PRIORITIES`): search/filters and the view switch stay in the row, saved views, group by, columns, export and host `toolbar_buttons` collapse. It is a fixed `matchMedia` threshold, not measurement, and all the state lives in the DOM (targets + data attributes) so a Turbo reconnect or a turbo-stream replace cannot strand a control inside the menu; `turbo:before-cache` always caches the expanded layout. Expanding re-sorts each group by priority, so the toolbar order is now defined by `OVERFLOW_PRIORITIES` rather than by the template. The `⋯` is not rendered at all when nothing is collapsible, and it is served hidden so it never flashes an empty menu. Two visual consequences: the DataTable view switch now defaults to `icon_only: :responsive` (new `Bali::ViewSwitch` mode that collapses only the label below `sm` while keeping `title`/`aria-label`, so the buttons never lose their accessible name), and the collapsible controls' labels are marked `toolbar-control-label` so the new `data_table/index.css` can bring them back inside the menu, where there is room — without it the `⋯` was a stack of anonymous icons. Dropdowns nested in the menu render in flow (`position: static`) because absolutely positioned ones escaped the viewport on a phone.
