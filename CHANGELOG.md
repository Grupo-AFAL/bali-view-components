...
- **`Bali::Icon::DefaultIcons` desaparece — 1,580 líneas y 166 SVGs de compatibilidad que ya no servían a nadie.** Era el quinto y último paso del pipeline de resolución, el rotulado "full backwards compatibility". Medido contra los 167 nombres que podía servir: **137 los interceptaba antes el mapa a Lucide y 28 el conjunto `KeptIcons`**, ambos pasos anteriores del pipeline — así que para 165 de ellos el fallback era código inalcanzable desde la migración a Lucide y borrarlo no cambia un pixel. Solo **dos** nombres seguían llegando hasta ahí y pierden su glifo: `money-bill-wave` (usa `banknote`, o `hand-coins` si lo que importaba era el pago y no el efectivo) y `question-circle` (usa `circle-help`). El segundo merece un grep: el mapa a Lucide sí tiene entrada para ese icono, pero bajo la clave `question_circle` — la única con guion bajo de toda la tabla, herencia del hash de v1 —, de modo que la grafía con guion bajo sigue funcionando y la que respeta la convención del resto de la librería, no.

  **La superficie grande es otra, y no aparece si buscas esos dos nombres.** El paso borrado no buscaba el nombre tal cual: lo pasaba a mayúsculas y cambiaba guiones por guiones bajos para construir un nombre de constante, así que `arrow_left`, `ARROW-LEFT` y `Arrow_Left` resolvían al mismo SVG que `arrow-left`. Los tres pasos que quedan comparan **exacto**: minúsculas, guiones, tal cual. En la práctica, la grafía snake_case de todo icono multi-palabra ahora lanza excepción — **73 nombres**, listados uno por uno en la [guía de migración](docs/guides/migration-v2-to-v3.md), y para 72 de ellos el arreglo es escribir el nombre con guiones. `money_bill_wave` es el único sin equivalente.

  El mensaje de `IconNotAvailable` cierra el hueco. Ya sugería nombres parecidos, pero los comparaba literalmente, así que `arrow_left` no encontraba nada y el mensaje degradaba a un enlace a lucide.dev. La comparación ahora ignora la diferencia entre guion y guion bajo: `arrow_left` responde `Did you mean: arrow-left?`.

  **`Bali::Icon::Options` se estrecha en consecuencia.** `Options.icons` eran los 166 SVGs legacy fusionados con `Bali.custom_icons`; ahora son los 28 iconos conservados fusionados con `Bali.custom_icons` — lo que Bali embarca como markup literal. Un nombre servido por Lucide no tiene SVG propio hasta que lucide-rails lo renderiza a un tamaño, así que `Options.find('user')` lanza `IconNotAvailable` donde antes devolvía el glifo FontAwesome viejo; `Options` nunca fue la ruta de render, para eso está `Bali::Icon::Component`. Los 28 SVGs conservados salen de Ruby a `app/components/bali/icon/svg/<name>.svg`, un archivo por nombre y byte por byte el mismo markup: `kept_icons.rb` sigue siendo un registro legible en vez de un blob de 570 líneas, los SVGs quedan abribles por quien los dibuja, y el glob `app/**/*` del gemspec ya los embarca.
