...
- **Los cinco page components pasan a tener UNA superficie, y vive en `Bali::PageComponents::Shared`.** Hasta v2 el concern compartía exactamente una cosa —la barra de acciones y su `⋯`— y todo lo demás divergía. El inventario real, que es lo que hacía falta para verlo:

  | | DashboardPage | DocumentPage | FormPage | IndexPage | ShowPage |
  |---|---|---|---|---|---|
  | `back:` | **no** | sí | sí | sí | sí |
  | `max_width:` | 4 claves, default `2xl` | **no** | 5 claves, default `md` | **no** | **no** |
  | slot `nav` | sí | **no** (`subheader`) | **no** | sí | sí |
  | slot `title_tags` | **no** | sí | **no** | **no** | sí |
  | slot del cuerpo | `body` | **`preview`** | `body` | `body` | `body` |
  | slot `sidebar` | **no** | **no** | sí | **no** | sí |
  | hueco cuerpo/encabezado | ninguno | `mt-6` | `mt-6` | `mt-4` | `mt-6` |
  | tamaño del subtítulo | `text-sm` | `text-base` | `text-sm` | `text-sm` | `text-base` |

  Las dos tablas de `max_width:` no coincidían: `:md` no existía en DashboardPage, `:"2xl"` no existía en FormPage, y en los otros tres el argumento reventaba con `ArgumentError` — el mismo símbolo significaba anchos distintos según a quién se lo pasaras. Ahora hay **una** tabla de seis claves (`sm`/`md`/`lg`/`xl`/`2xl`/`full`) en el concern, y lo único que sigue difiriendo entre componentes es el DEFAULT: `:"2xl"` en DashboardPage, `:md` en FormPage, `:full` en los otros tres. `:full` es deliberadamente un no-op (`mx-auto max-w-full` sobre un `div` de bloque no mueve nada), así que los tres que nunca tuvieron contenedor heredan la opción sin cambiar de layout — medido en el navegador a 390px y a 1440px, con las seis claves.

  `back:`, `title_tags`, `nav`, `body`, `sidebar` y un `render_page_header` compartido suben al concern, así que **DashboardPage gana `back:`; FormPage y DocumentPage ganan `nav`; DashboardPage, FormPage e IndexPage ganan `title_tags`; y los cinco ganan `sidebar`**. El grid de cuerpo + barra lateral que ShowPage y FormPage tenían copiado literal —las mismas seis clases en los dos archivos— es ahora `render_body_with_sidebar` con `sidebar_width:` (`:default` un tercio, `:narrow` un cuarto, `:wide` la mitad); lo único que FormPage no comparte es que envuelve el cuerpo en una `Card`, y eso es un método redefinido de tres líneas.

  **Qué te cambia el render si actualizas.** Tres cosas, todas de espaciado o tamaño, ninguna estructural: el cuerpo de IndexPage pasa de `mt-4` a `mt-6`, que es el hueco de los otros cuatro; el subtítulo de ShowPage y DocumentPage pasa de `text-base` a `text-sm`, que es el `SUBTITLE_CLASSES` que el propio PageHeader declara y que los otros tres ya usaban (venían por el slot `with_subtitle`, cuyo tamaño lo deriva del tag `h6`, en vez de por el argumento del constructor); y la grilla de stats de DashboardPage suelta su `mb-6` porque ahora el `mt-6` del cuerpo hace ese hueco — un dashboard sin `body` deja de arrastrar 24px de aire al final. El resto del HTML es idéntico salvo el orden de las clases del `h3` del título y el whitespace, que cambia porque el encabezado se arma desde Ruby y no desde cinco plantillas ERB.

  **`DocumentPage` renombra su slot `preview` a `body`.** Era el único de los cinco con otro nombre para lo mismo. `with_preview` sigue funcionando y avisa por `Bali.deprecator` hasta 4.0, así que un host que no lo renombre ve un warning, no una página en blanco. El slot `subheader` no se toca: se pinta a lo ancho, antes del layout de paneles, y no es lo mismo que un `nav`.

  **`DashboardPage#with_stat` pinta un `Bali::StatCard` de verdad.** Había dos tarjetas de estadística con dos diseños en el mismo repo (tres con `InfoLevel`): la de DashboardPage era markup inline con su propia tabla de colores. La que queda es StatCard, así que el rótulo pasa a versalitas `text-xs`, la cifra a `text-3xl`, el icono a un círculo con fondo del color, y `change:` cae en el slot `footer`. Es un cambio visible en cualquier dashboard; la firma de `with_stat` no cambia. De paso, `StatCard#icon_name` deja de ser obligatorio — `with_stat` siempre permitió no pasar icono, y una tarjeta sin icono es una tarjeta, no un error.

  **El contrato del concern se prueba una vez, recorriendo los cinco.** `test/bali/components/page_components/shared_contract_test.rb` es un test por regla —el encabezado compartido, la posición del `nav`, las seis claves de `max_width` sobre los cinco, los tres `sidebar_width`, el `⋯` de acciones secundarias, el menú de exportar— iterando la lista de componentes en vez de copiarse cinco veces. La cobertura copiada es exactamente lo que había dejado a ShowPage y DashboardPage sin una sola prueba del `⋯` de v3 mientras IndexPage tenía seis.

  **Constantes que se mueven**: `Bali::DashboardPage::Component::MAX_WIDTHS` y `Bali::FormPage::Component::MAX_WIDTHS` son ahora `Bali::PageComponents::Shared::MAX_WIDTHS`, y `Bali::DashboardPage::Component::STAT_ICON_COLORS` desaparece porque la paleta que manda es `Bali::StatCard::Component::COLORS`. `Bali::DashboardPage::Stat` sigue existiendo con los mismos cinco miembros.

  **Lo que este cambio NO toca, a propósito**: la jerarquía de encabezados del PageHeader (sigue emitiendo `h3` y un `h6` vacío cuando no hay subtítulo) y su apilado responsive. Eso es #685.
...
- **`Bali::Level` y `Bali::InfoLevel`** avisan por `Bali.deprecator` y se eliminan en 4.0. Ninguno se borra ahora: el aviso sale al construirlos y el render es el de siempre. `Level` es una fila flex con `justify-between` y nada más, así que `<div class="flex justify-between items-center gap-4">` hace lo mismo sin un componente de por medio — y para el encabezado de una página está `Bali::PageHeader`, que es lo que Level venía sosteniendo. `InfoLevel` es el tercer diseño de tarjeta de estadística del repo: cada `InfoLevel::Item` es un rótulo arriba y una cifra grande abajo, o sea un `StatCard` con otra tipografía; la sustitución es una grilla de `Bali::StatCard`, que es también lo que `DashboardPage#with_stat` pinta desde v3.

  **PageHeader todavía usa Level por dentro y silencia el aviso** (`Bali.deprecator.silence` alrededor del `.new`). No es un descuido: si el warning saliera de ahí, un host que nunca escribió `Bali::Level` recibiría una deprecación por cada encabezado de página, sobre una decisión que no es suya y que no puede accionar. Reemplazar ese Level por flex plano es trabajo del propio PageHeader. Hay un test que falla si el aviso vuelve a escaparse.

- **El slot `preview` de `Bali::DocumentPage`** avisa y se elimina en 4.0. Renómbralo a `with_body`; ver la entrada de page components arriba.
