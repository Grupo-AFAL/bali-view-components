...
class DashboardWidget
  ...
  # `dashboard_widgets` — `DashboardWidget`, the AR model this class is nested
  # under, is a row and nothing more; see its own comments.
  #
  ...
  # `Bali::SavedView::Store`. There is no separate `dashboards` table backing
  # that scope: owner + context + dashboard_key is pure join identity, and a
  # table for it would buy only an owner, a context and two timestamps —
  # bought with a migration and an extra read on every request, for a name we
  # can have for free by scoping `DashboardWidget` rows directly. Promote it
  # the day a dashboard has state of its own.
  #
  ...
  # that object instead of this one — the same seam `Bali::SavedView::Store`
  # proves (a phase-2 team-shared implementation is OTRA clase con este mismo
  # contrato):
  #
  ...
  #
  # Bali ships this contract and nothing that implements it — the widget
  # contract, the card and the grid are the engine's, persistence is always
  # the host's. This class is a WORKED EXAMPLE of that contract, not the only
  # possible one; see `docs/guides/widgets.md` for the contract in full and
  # for the five behaviours a naive implementation gets wrong.
  #
  ...
  class Store
    ...
    def rows
      DashboardWidget.where(owner: owner, context: context,
    ...
    # because it is Postgres-only and this dummy app runs on SQLite, where
    # `.lock` is a no-op besides — see `docs/guides/widgets.md`.
    def lock_rows
      rows.lock.pluck(:id)
    end
